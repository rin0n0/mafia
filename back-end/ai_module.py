...
class AINarrator:
    ...
    def _generate_prompt(self, context: AIContext, event_type: str, event_data: Dict) -> tuple[str, List[types.Content]]:
        ...
        system_instruction = (
            ...
            "Твоя задача — красочно описывать игровые события. Ты должен быть креативным и активно использовать описания персонажей. Учитывай живы ли они или они уже выбыли из игры"
            "Ты можешь использовать шутки на основе доступного контекста, если это будет уместо и покажет тебя как более профессионального ведущего"
    ...
    def _get_user_query(self, event_type: str, event_data: Dict) -> str:
        ...
        queries = {
            "game_start": f"Игра начинается. Напиши приветственное атмосферное вступление для погружения игроков в сеттинг. {length_constraint}",
            "night_kill": f"Произошло событие 'убийство ночью'. Жертва — {victim}. Опиши эту сцену в рамках сеттинга. {length_constraint}",
            "night_save": f"Произошло событие 'спасение ночью'. Мафия пыталась убить игрока {victim or 'некую цель'}, но его спасли. Опиши это в рамках сеттинга. {length_constraint}",
            "night_no_kill": f"Этой ночью никто не умер. Опиши эту тихую, но напряженную ночь сеттинга. {length_constraint}",
            "lynch_victim": f"Произошло событие 'казнь'. Решили казнить игрока {victim}. Опиши сцену казни в рамках сеттинга. {length_constraint}",
            "lynch_tie": f"Дневное голосование за казнь закончилось ничьей. Опиши, как голоса разделились. {length_constraint}",
            "joke_voting_start": "Сейчас начнется шуточное голосование. Придумай и задай игрокам безобидный и весёлый вопрос связанный с сеттингом (в поле 'summary'), ответом на который будет имя одного из них.",
            "joke_vote_result": f"Шуточное голосование завершилось. Самым подозрительным посчитали игрока {victim}. Опиши итог с учетом сеттинга и описаний игроков. {length_constraint}",
            "joke_vote_tie": f"Шуточное голосование закончилось ничьей. Опиши, как мнения разделились. {length_constraint}",
            "day_start": f"Начинается новый день. Опиши, как все просыпаются, опираясь на сеттинг и на описания каких-то игроков, которые пока что остались вживых. Обязательно добавь предложение 'Начинается обсуждение итогов ночи.' {length_constraint}",
            "night_start": f"Наступает ночь. Опиши, как всё погружается во тьму с учётом сеттинга. {length_constraint}",
            "voting_start": f"Начинается дневное голосование. Опиши призыв к голосованию и обязательно добавь предложение 'Начинается голосование.'. {length_constraint}",
             "game_over": f"Игра закончена. Победили: {winner}. Опиши эпилог этой истории. Что стало с игровым окружением? {length_constraint}",
        }
